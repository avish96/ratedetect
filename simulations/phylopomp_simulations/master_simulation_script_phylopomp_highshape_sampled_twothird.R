## Master template script used for simulation-based power analysis for phylopomp trees
##
...
tree_type <- 'phylopomp_2host'
scenario <- 'kscale_highshape_sampled_twothird'

## Clock simulation - parameter values (a - shape, b - scale)
a <- 9.81 # Worobey et al. (2014) H7 Avian HA fit parameter values, returned by treedater maximum likelihood fit
b <- 0.590/1716 # scaled by sequence length from H7 HA alignment to get desired values 
# a <- a/4 # If low-shape
scaler <- 1 # Scaling factor 

# Empirically-derived sampling weights
normalized_weights <- setNames(c(0.0381,0.0490,0.0630,0.0811,0.104,0.134,0.172,0.221,0.285,0.366,0.471,0.605,0.778,1.00),1:14)

## Effect sizes for power analysis
klist <- c(1,1.25,1.5,2,3,5,7.5,10,20,30,40)

#### ENSURE TO FIRST SET UP FILE PATHS AS BELOW ###

# Set paths
timepath <- paste0('./',tree_type,'_trees_time/')
ratepath <- paste0('./phylopomp_2host_sims/',scenario,'/',tree_type,'_rate/')
subpath <- paste0('./phylopomp_2host_sims/',scenario,'/',tree_type,'_subs/')
timeinfpath <-  paste0('./phylopomp_2host_sims/',scenario,'/',tree_type,'_time_inferred/')
rateinfpath <-  paste0('./phylopomp_2host_sims/',scenario,'/',tree_type,'_rate_inferred/')
powerpath <-  paste0('./phylopomp_2host_sims/',scenario,'/PowerAnalysis/')
errorpath <-  paste0('./phylopomp_2host_sims/',scenario,'/Errors/')

# Create paths if not already (time should already exist)
...
y <- foreach (
  ...
  v_t$edge.length <- v_t$edge.length/365.25
  v_t$edge.length <- v_t$edge.length*scaler
  ...
  # Sampling
  dropped <- sample(v_p1$tip.label,length(v_p1$tip.label)/1.5,prob=rev(sort(rep(normalized_weights, length.out = length(v_p1$tip.label)))))
  v_p1_s <- drop.tip(v_p1,dropped)
  v_t_s <- drop.tip(v_t,dropped)

  # Write trees
  write.tree(v_r,paste0(ratepath,tree_type,"_k",k,"_",j,"_rate.nwk"))
  write.tree(v_p1,paste0(subpath,tree_type,"_k",k,"_",j,"_subs.nwk"))
  write.tree(v_p1_s,paste0(subpath,tree_type,"_k",k,"_",j,"_subs_sampled.nwk"))
  save(v_p1_s,v_t_s,dropped,file=paste0(subpath,tree_type,"_k",k,"_",j,"_time_subs_sampled.RData"))
